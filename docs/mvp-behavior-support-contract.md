# HoursFlow CTA MVP behavior and support contract
...
Consequently, cached HTML may remain stale for up to the page-cache TTL. Documentation MUST disclose this limitation. A site requiring a maximum staleness of N minutes must configure a TTL of no more than N minutes or arrange boundary purges in its caching system; HoursFlow performs neither operation. The plugin itself MUST NOT cache the computed open/closed result across requests.
...
- Uninstall permanently removes every HoursFlow option and schema/version value. The MVP creates no custom tables, content, user metadata, or transient business data.
...
- Every plugin-authored user-facing PHP or JavaScript string MUST be translatable with the `hoursflow` text domain, including block-editor and validation messages. Weekday labels in administration MUST use WordPress locale data; stored weekday keys and `HH:MM` values remain locale-independent. Administrator-authored CTA copy is displayed as entered and is not automatically translated.
